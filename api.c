...
Dictionary dictionary_create(char *dict_name, char * pp_name) {
    Dictionary dict;
    ...
    dict->postprocessor      = post_process_open(dict->name, dict->post_process_filename);
...
void linkage_post_process(Linkage linkage, Postprocessor * postprocessor) {
	...
	if (subl->violation != NULL) {
        ...
        if (linkage->info.improper_fat_linkage) {
            ...
            pp = post_process(postprocessor, opts, sent, subl);
