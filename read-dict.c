...
Dict_node * insert_dict(Dictionary dict, Dict_node * n, Dict_node * dn_new) {
...
/* and right fields of it are NULL                                       */
    ...
    if (n == NULL) return dn_new;
    comp = dict_compare(dn_new->string, n->string);
    if (comp < 0) {
        n->left = insert_dict(dict, n->left, dn_new);
    } else if (comp > 0) {
        n->right = insert_dict(dict, n->right, dn_new);
    } else {
...
void set_parent_of_node(Dictionary dict,
            Dict_node *p, Dict_node * del, Dict_node * dn_new) {
    if (p == NULL) {
    dict->root = dn_new;
    } else {
	if (p->left == del) {
...
int open_dictionary(Dictionary dict) {
    if ((dict->fp = dictopen(NULL, dict->name, "r")) == NULL) {
	return 0;
    }
    return 1;
}
