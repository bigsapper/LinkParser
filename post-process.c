...
static int check_domain_nesting(Postprocessor *pp, int num_links)
{
  ...
  List_o_links * lol;
  int i;
  for (d1=pp->domain_array; d1 < pp->domain_array + pp->N_domains; d1++) {
    for (d2=d1+1; d2 < pp->domain_array + pp->N_domains; d2++) {
...
static void free_post_processing_structures(Postprocessor *pp)
{
  /* gets called after every invocation of post_process() */
  int w, d;
  for (w=0; w<pp->length; w++) 
    free_List_o_links(pp->word_links[w]);
  for (d=0; d<pp->N_domains; d++) 
    {
      free_List_o_links(pp->domain_array[d].lol);
      free_D_tree_leaves(pp->domain_array[d].child);
    }
  free_List_o_links(pp->links_to_ignore);
  pp->links_to_ignore = NULL;
}
			     ...
			     int w, pp_linkset *ls)
{
  List_o_links *lol;
  pp->visited[w] = TRUE;
  for (lol = pp->word_links[w]; lol != NULL; lol = lol->next) {
...
static int is_connected(Postprocessor *pp)
{
  ...
  for (i=0; i<pp->length; i++) 
    pp->visited[i] = (pp->word_links[i] == NULL);
...
apply_contains_one(Postprocessor *pp, Sublinkage *sublinkage, pp_rule *rule) 
{
  ...
  for (d=0; d<pp->N_domains; d++) 
    {
      for (dtl = pp->domain_array[d].child; 
	   dtl != NULL && 
      ...
      if (dtl != NULL) 
	{
	  ...
	  count=0;
	  for (dtl = pp->domain_array[d].child; dtl != NULL; dtl = dtl->next) 
...
apply_connected_without(Postprocessor *pp,Sublinkage *sublinkage,pp_rule *rule)
{
  /* Returns true if the linkage is connected when ignoring the links
     ...
     these links. */
  int i;
  memset(pp->visited, 0, pp->length*(sizeof pp->visited[0]));
  mark_reachable_words(pp, 0);
  for (i=0; i<pp->length; i++) 
...
apply_must_form_a_cycle(Postprocessor *pp,Sublinkage *sublinkage,pp_rule *rule)
{
  /* Returns TRUE if the linkage is connected when ignoring the links
    ...
    for (lol = pp->links_to_ignore; lol != NULL; lol = lol->next) {
...
apply_bounded(Postprocessor *pp,Sublinkage *sublinkage,pp_rule *rule)
{
  ...
  d_type = rule->domain;
  for (d=0; d<pp->N_domains; d++) {
    if (pp->domain_array[d].type != d_type) continue;
    lw = sublinkage->link[pp->domain_array[d].start_link]->l;
    for (lol = pp->domain_array[d].lol; lol != NULL; lol = lol->next) {
...
{
  /* fill in the pp->word_links array with a list of words neighboring each
  ...
  for (link=0; link<sublinkage->num_links; link++) 
    {
	  ...
	  lol->next = pp->links_to_ignore;
	  pp->links_to_ignore = lol;
	  lol->link = link;
      ...
      lol = (List_o_links *) xalloc(sizeof(List_o_links));
      lol->next = pp->word_links[sublinkage->link[link]->l];
      pp->word_links[sublinkage->link[link]->l] = lol;
      lol->link = link;
      lol->word = sublinkage->link[link]->r;
      
      lol = (List_o_links *) xalloc(sizeof(List_o_links));
      lol->next = pp->word_links[sublinkage->link[link]->r];
      pp->word_links[sublinkage->link[link]->r] = lol;
      lol->link = link;
...
{
  memset(pp->visited, 0, pp->length*(sizeof pp->visited[0]));/* set pp->visited[i] to FALSE */
  pp->domain_array[n].string = string;
  pp->domain_array[n].lol        = NULL;
  pp->domain_array[n].size       = 0;
  pp->domain_array[n].start_link = start_link;
}
...
static void add_link_to_domain(Postprocessor *pp, int link) 
{
  List_o_links *lol;
  lol = (List_o_links *) xalloc(sizeof(List_o_links));
  lol->next = pp->domain_array[pp->N_domains].lol;
  pp->domain_array[pp->N_domains].lol = lol;
  pp->domain_array[pp->N_domains].size++;
  lol->link = link;
}
...
static void build_domains(Postprocessor *pp, Sublinkage *sublinkage)
{
  ...
  for (link = 0; link<sublinkage->num_links; link++) {
    ...
    if (pp_linkset_match(pp->knowledge->domain_starter_links, s)) 
      {
	setup_domain_array(pp, pp->N_domains, s, link);
	...
	pp->N_domains++;
        assert(pp->N_domains<PP_MAX_DOMAINS, "raise value of PP_MAX_DOMAINS");
      } 
    else {
      if (pp_linkset_match(pp->knowledge->urfl_domain_starter_links,s)) 
	{
	  ...
	  bad_depth_first_search(pp,sublinkage,sublinkage->link[link]->r,
				 sublinkage->link[link]->l,link);
	  pp->N_domains++;
	  assert(pp->N_domains<PP_MAX_DOMAINS,"raise PP_MAX_DOMAINS value");
	}
      else 
  ...
  for (d=0; d<pp->N_domains; d++) {
    i = find_domain_name(pp, pp->domain_array[d].string);
    if (i==-1) 
       error("\tpost_process: Need an entry for %s in LINK_TYPE_TABLE",
           pp->domain_array[d].string);
    pp->domain_array[d].type = i;
  }
}
...
static void build_domain_forest(Postprocessor *pp, Sublinkage *sublinkage)
{
  ...
  DTreeLeaf * dtl;
  if (pp->N_domains > 0) 
    pp->domain_array[pp->N_domains-1].parent = NULL;
  for (d=0; d < pp->N_domains-1; d++) {
    for (d1 = d+1; d1 < pp->N_domains; d1++) {
      if (contained_in(&pp->domain_array[d],&pp->domain_array[d1],sublinkage))
	{
	  pp->domain_array[d].parent = &pp->domain_array[d1];
	  break;
	}
    }
    if (d1 == pp->N_domains) {
      /* we know this domain is a root of a new tree */
      pp->domain_array[d].parent = NULL;
  ...
  /* the parent links of domain nodes have been established.
     now do the leaves */
  for (d=0; d < pp->N_domains; d++) {
    pp->domain_array[d].child = NULL;
  }
  for (link=0; link < sublinkage->num_links; link++) {
    if (sublinkage->link[link]->l == -1) continue; /* probably not necessary */
    for (d=0; d<pp->N_domains; d++) {
      if (link_in_domain(link, &pp->domain_array[d])) {
	dtl = (DTreeLeaf *) xalloc(sizeof(DTreeLeaf));
	dtl->link = link;
	dtl->parent = &pp->domain_array[d];
	dtl->next = pp->domain_array[d].child;
	pp->domain_array[d].child = dtl;
	break;
      }
    }
  }
...
Postprocessor * post_process_open(char *dictname, char *path)
{
  ...
  */
  Postprocessor *pp;
  if (path==NULL) { 
    fprintf(stderr, "Not using postprocessing\n"); 
    return NULL; 
  }
		      ...
		      Sentence sent, Sublinkage *sublinkage) 
{
  ...
  build_type_array(pp);
  free_post_processing_structures(pp);  
  return pp->pp_node;
}
